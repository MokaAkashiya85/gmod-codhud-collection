...
local function weaponinfo(...)
    ...
    if clip >= 0 then
        ...
        if primCount >= 1000 then
            DrawSqueezedText(primCount, "WaW_Res_3D", barX + barW + CoDHUD_SX(CFG.RES4_X), barY + CoDHUD_SY(CFG.RES4_Y), resCol, CoDHUD_S(-2.5), CoDHUD_S(-2.5), 2)
        elseif primCount >= 100 then
            DrawSqueezedText(primCount, "WaW_Res_3D", barX + barW + CoDHUD_SX(CFG.RES3_X), barY + CoDHUD_SY(CFG.RES3_Y), resCol, CoDHUD_S(-2.5), CoDHUD_S(-2.5), 2)
        else
            DrawSqueezedText(primCount, "WaW_Res_3D", barX + barW + CoDHUD_SX(CFG.RES_X), barY + CoDHUD_SY(CFG.RES_Y), resCol, CoDHUD_S(0), CoDHUD_S(0), 2)
        end
    end
...
local function subtitles(...)
    ...
    for i = #SubtitleQueue, 1, -1 do
		...
		local speaker = CoDHUD_UpperText(data.speaker)
		local text = data.text
		
		surface.SetFont("WaW_KillfeedFont")
        ...
        if GetConVar("codhud_enable_subtitles_callsign"):GetBool() then
			if GetConVar("codhud_enable_subtitles_callsign_alignment"):GetBool() then
				draw.SimpleTextOutlined( speaker, "WaW_KillfeedFont", centerX, y - textH, Color(255,0,0,alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, outlined and 1 or 0, Color(0,0,0,alpha) )
			else
				draw.SimpleTextOutlined( speaker .. ": ", "WaW_KillfeedFont", centerX - (textW * 0.5), y, Color(255,0,0,alpha), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, outlined and 1 or 0, Color(0,0,0,alpha) )
			end
		end

        draw.SimpleTextOutlined( text, "WaW_KillfeedFont", centerX, y, Color(255,255,255,alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, outlined and 1 or 0, Color(0,0,0,alpha) )
    end
end
