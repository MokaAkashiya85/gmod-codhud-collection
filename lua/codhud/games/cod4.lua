...
local function levelup( ... )
    ...
    CoDHUD_HeaderQueue.Push({
		...
		sfx = "codhud/hud/mp_levelup_final.mp3",
        fonts = {
...
local function weaponinfo(...)
    ...
    if altType ~= -1 then
		if altType ~= primType and altType ~= game.GetAmmoID("Grenade") then
			...
			DrawSqueezedText(altAdd, "CoD4_Ammo_Alt", (ScrW() * 0.5) - CoDHUD_SX(CFG.ALT_TEXT_X), barY + CoDHUD_SY(CFG.ALT_TEXT_Y), altCol, CFG.ALT_TEXT_SQ, CFG.ALT_TEXT_SQ1, 2)
